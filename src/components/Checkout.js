...
const Checkout = () => {
  ...
  const getAddresses = async (token) => {
    // console.log('This is address token', token)
    if (!token) {
      // console.log('Token is not thwrw ')
      return
    }
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const newAddr = {address : newAddress.value}
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      const res = await axios.delete(
        ...
        setAddresses({...addAddress, all: res.data})
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const performCheckout = async (token, items, addresses) => {
      if(validateRequest(items, addresses)) {
        try{
          ...
          history.push('/thanks')
          enqueueSnackbar(res.response.data.message, { variant: "success" });
        }
        catch(e) {
          if (e.response) {
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      if (productsData && cartData) {
        ...
        // console.log('this is tokeeeen',token)
        getAddresses(token)
      }
    };
    onLoadHandler();
                ...
                <Box>
                  {addresses.all.map((add) => {
                    return (
                      <Box
                        className={
                          add._id === addresses.selected
                            ? "address-item selected"
                            : "address-item not-selected"
                        }
                        onClick={() => handleAddressOnClick(add._id)}
                        key={add._id}
                      >
                        ...
                        <Button
                          variant="text"
                          startIcon={<Delete />}
                          onClick={async () => deleteAddress(token, add._id)}
                        >
                          DELETE
                        </Button>
                      </Box>
